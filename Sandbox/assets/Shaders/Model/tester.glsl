...
vec4 CalculatePointLight(PointLight light, CommonData data)
{
	...
	vec3 lightDirection = normalize(vec3(light.position) - fs_in.FragPos) * vec3(-1.0);
	vec3 lightColour = vec3(light.diffuse) * diffTex1;
	vec2 lightBias = vec2(0.1f, 1.0f);
	float brightness = max(dot(-lightDirection, normalize(fs_in.Normal)), 0.0);
	
	float level = brightness * levels;
	brightness = level / levels;

	return vec4((lightColour * lightBias.x) + (brightness * lightColour * lightBias.y), 1.0);
}
